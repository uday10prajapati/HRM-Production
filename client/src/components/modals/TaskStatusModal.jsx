        ...
        if (selectedUser?.role?.toLowerCase() === "engineer") {
          ...
          if (res.data.success) {
            ...
            const userCalls = res.data.calls.filter(
              (c) => String(c.id) === String(selectedUser.id) || String(c.user_id) === String(selectedUser.id)
            );
            // Map to unified structure
            setTaskList(userCalls.map(c => ({
              id: c.call_id || c.id,
              title: c.dairy_name || "Assigned Call",
              description: c.problem || c.description,
              status: c.status,
              customerMobile: c.mobile_number,
          ...
          // Use standard tasks
          setTaskList(selectedUser?.tasks || []);
        }
      } catch (err) {
        console.error("Failed to fetch tasks/calls", err);
        setTaskList(selectedUser?.tasks || []);
      } finally {
        setLoading(false);
      }
                ...
                >
                  <div className="p-5">
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <span
                        className={`ml-4 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${task.status?.toLowerCase() === "completed"
