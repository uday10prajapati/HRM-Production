...
const AllUsers = () => {
 ...
 const [formData, setFormData] = useState({
  ...
  email: "",
  role: "",
  leaveBalance: 20,  // Default 20 leaves
  attendanceStatus: "",
  ...
  const handleAddUser = () => {
  setFormData({ name: "", email: "", role: "", leaveBalance: 20, attendanceStatus: "", password: "" });
...
const handleAddSubmit = async () => {
  try {
    // Ensure leaveBalance is a number
    const dataToSend = { ...formData, leaveBalance: Number(formData.leaveBalance) || 20 };
            ...
            <div className="overflow-x-auto rounded-lg border border-gray-300 bg-white shadow">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-100">
                  <tr>
                    {["ID", "Name", "Email", "Role", "Leave Balance", "Attendance Status", "Actions"].map(
                      (header) => (
                        <th
                          key={header}
                ...
                <tbody className="divide-y divide-gray-200">
                  {users.map((user) => (
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-900">{user.leaveBalance ?? "N/A"}</td>
